...
export function ExecutivoView({ profile, states, usersList, decrees, templates, projects, actions }: any) {
  ...
  // FIX: Busca a jurisdição inteligente (reconhece o alias 'federal')
  const myJurisdiction = states.find((s:any) => s.id === profile?.jurisdictionId) 
                         || states.find((s:any) => s.type === 'federal') 
                         || { id: 'federal', name: 'União Federal' };
  ...
  return (
    <div className="space-y-8">
              ...
              <div key={dec.id} className="bg-gray-900 p-5 border-l-4 border-blue-500 rounded-lg">
                 ...
                 {dec.actions && dec.actions.length > 0 && (
                   <div className="mt-4 p-3 bg-black/20 rounded border border-gray-700">
                     <p className="text-xs font-bold text-gray-500 uppercase mb-2">Atos Administrativos (Diário Oficial):</p>
                     {dec.actions.map((a, i) => (
      ...
      {/* CONTROLE DE PASTAS (Agora vísivel para todos, mas só o Ministro opera) */}
      <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
        <h2 className="text-2xl font-bold text-white mb-6">Controle de Pastas & Tesouro</h2>
        {states.map((state: RpgData) => {
          // FIX: Verifica se o usuário é ministro DESTE estado (Lida com o alias 'federal')
          const isMin = profile?.role === 'ministro' && 
                        (profile?.jurisdictionId === state.id || (profile?.jurisdictionId === 'federal' && state.type === 'federal'));
            ...
            <div key={state.id} className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {pastasArray.map((pastaName: string) => {
                  // FIX: O dono também respeita o alias 'federal' para não mostrar "Vago" indevidamente
                  ...
                  const pastasArray = Array.from(new Set([
                    ...Object.keys(TAXONOMY),
                    ...Object.keys(state.allocatedBudget || {})
                  ]));
                  ...
                  return (
                    <div key={pastaName} className="bg-gray-900 p-4 border border-gray-700 rounded-lg relative overflow-hidden">
                      ...
                      {/* O botão de Investir só aparece se for O ministro certo DESTA pasta */}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-full max-w-2xl border border-yellow-600 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="bg-gray-900 rounded-lg border border-gray-700 p-4 mb-6">
               ...
               <div className="space-y-3">
                 {/* Proteção caso o projeto chegue sem artigos de texto */}
                 {(!modalSancao.artigos || modalSancao.artigos.length === 0) && (
                   <p className="text-gray-400 text-sm italic">Projeto sem redação articulada. Clique em Sancionar.</p>
                 )}
      ...
      {/* MODAL DE NOVO DECRETO (Com Lista de Nomeações) */}
      {modalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-full max-w-lg border border-gray-600 shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold mb-4 text-white border-b border-gray-700 pb-2">Redigir Decreto Executivo</h3>
            ...
            {decreto.templateId && (
              <>
                ...
                <textarea rows={4} placeholder="Corpo do Decreto" value={decreto.content} onChange={e => setDecreto({...decreto, content: e.target.value})} className="w-full bg-gray-900 text-white border border-gray-700 p-3 rounded mb-4 outline-none" />
                ...
                <div className="flex gap-2">
                   ...
                   <button onClick={() => { actions.publishDecreto(decreto, actionsList); setModalOpen(false); setActionsList([]); }} className="flex-1 bg-blue-600 text-white py-3 rounded font-bold hover:bg-blue-500 shadow-lg transition">Assinar e Publicar</button>
                </div>
              </>
            )}
            {!decreto.templateId && <button onClick={() => setModalOpen(false)} className="w-full mt-4 py-3 text-gray-400 hover:bg-gray-700 rounded transition">Cancelar Operação</button>}
          </div>
        </div>
      )}
    </div>
