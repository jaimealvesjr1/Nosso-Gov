...
import { FileSignature, FileBadge, Trash2 } from 'lucide-react';
import { RpgProject, DocTemplate, TAXONOMY, DecreeAction, MacroArea } from '../types';
import { getYear } from '../components/UI';

export function ExecutivoView({ profile, states, decrees, projects, templates, actions, gameTime }: any) {
  const [modalOpen, setModalOpen] = useState(false);
  const [modalSancao, setModalSancao] = useState<RpgProject | null>(null);
  const [alteracoesArtigos, setAlteracoesArtigos] = useState<Record<number, {isVetoed: boolean, newText: string}>>({});
  ...
  const execTemplates = templates.filter((t: DocTemplate) => t.branch === 'executivo' && !t.isArchived);
  ...
  const handleTemplateChange = (e: any) => {
    ...
    if(tpl) {
      setDecreto({ ...decreto, templateId: tpl.id, category: tpl.category, stateId: myJurisdiction.id, title: `${tpl.name} N° ${nextNum}/${gameTime.year || getYear()}`, templateBody: tpl.bodyText || "{{TEXTO_JOGADOR}}", userText: '' });
    }
  };
          ...
          <div className="space-y-3">
            {projetosAguardando.map((p: RpgProject) => (
              <div key={p.id} className="bg-gray-900 p-4 rounded border border-yellow-900/50 flex justify-between items-center">
                ...
                <button onClick={() => {setModalSancao(p); setAlteracoesArtigos({});}} className="bg-yellow-600 hover:bg-yellow-500 text-white px-4 py-2 rounded font-bold transition">Analisar</button>
              </div>
            ))}
          </div>
        </div>
         ...
         <div className="space-y-4">
            {decrees.map((dec: any) => (
              <div key={dec.id} className="bg-gray-900 p-5 border-l-4 border-blue-500 rounded-lg relative group">
                 {profile?.role === 'admin' && <button onClick={() => actions.deleteDocument('decrees', dec.id)} className="absolute top-4 right-4 text-red-500 opacity-0 group-hover:opacity-100 transition"><Trash2 size={18}/></button>}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-full max-w-2xl border border-yellow-600 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="bg-gray-900 rounded-lg border border-gray-700 p-4 mb-4">
               ...
               <div className="space-y-4">
                 {modalSancao.artigos?.map((art: any) => {
                   const estadoAlt = alteracoesArtigos[art.id] || { isVetoed: false, newText: art.text };
                   return (
                     <div key={art.id} className={`p-4 rounded border transition-colors ${estadoAlt.isVetoed ? 'bg-red-900/20 border-red-900/50' : 'bg-gray-800 border-gray-700'}`}>
                        <div className="flex justify-between items-start mb-2">
                          <span className="font-bold block text-sm text-yellow-500">Art. {art.id}º</span>
                          <label className="flex items-center cursor-pointer text-xs font-bold text-red-400">
                            <input type="checkbox" checked={estadoAlt.isVetoed} onChange={(e) => setAlteracoesArtigos({...alteracoesArtigos, [art.id]: { ...estadoAlt, isVetoed: e.target.checked }})} className="mr-2 w-4 h-4 accent-red-500" />
                            VETAR ARTIGO
                          </label>
                        </div>
                        <textarea rows={2} value={estadoAlt.newText} disabled={estadoAlt.isVetoed} onChange={e => setAlteracoesArtigos({...alteracoesArtigos, [art.id]: { ...estadoAlt, newText: e.target.value }})} className={`w-full text-sm p-2 rounded outline-none border ${estadoAlt.isVetoed ? 'bg-gray-900 border-gray-800 text-gray-600 line-through' : 'bg-gray-900 border-gray-600 text-gray-200 focus:border-yellow-500'}`} />
                     </div>
                   )
                 })}
               </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setModalSancao(null)} className="flex-1 py-3 text-gray-400 hover:bg-gray-700 rounded transition">Adiar</button>
              <button onClick={() => { actions.analisarSancao(modalSancao.id, alteracoesArtigos); setModalSancao(null); setAlteracoesArtigos({}); }} className="flex-1 bg-green-600 text-white py-3 rounded font-bold hover:bg-green-500">Sancionar / Vetar</button>
            </div>
          </div>
        </div>
      )}
