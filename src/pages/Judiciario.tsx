...
export function JudiciarioView({ profile, decisions, usersList, states, templates, actions, gameTime }: any) {
  ...
  // FIX: Adicionado userText e templateBody
             ...
             {sentenca.title && (
               <>
                 ...
                 <div className="bg-gray-950 p-3 rounded border border-gray-800 mb-2">
                    <p className="text-xs text-gray-500 uppercase font-bold mb-1">O texto abaixo será formatado automaticamente com o cabeçalho do Tribunal.</p>
                 </div>
                 <textarea rows={6} placeholder="Digite a decisão judicial..." value={sentenca.userText} onChange={e => setSentenca({...sentenca, userText: e.target.value})} className="w-full bg-gray-900 border border-gray-700 text-white p-3 rounded mb-4 outline-none" />
                 ...
                 <div className="flex gap-2">
                    ...
                    }} className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-white py-3 rounded font-bold shadow-lg transition">Publicar Sentença</button>
                 </div>
               </>
             )}
           </div>
       ...
       {modalTse && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-full max-w-md border border-emerald-600 shadow-2xl">
               ...
               <div className="mb-4 bg-gray-900 p-3 rounded border border-gray-700">
                 ...
                 {diplomados.map((d, idx) => (
                   <div key={idx} className="flex justify-between items-center text-sm border-b border-gray-800 py-2 last:border-0">
                     <span className="text-gray-300 font-bold">{usersList.find((u:any)=>u.id===d.userId)?.discordUsername} <span className="text-emerald-400 font-normal ml-1">({formatRole(d.role)})</span></span>
             ...
             <button onClick={() => { if(diplomaItem.userId) { setDiplomados([...diplomados, diplomaItem]); setDiplomaItem({...diplomaItem, userId: ''}); } }} className="w-full bg-gray-700 hover:bg-gray-600 text-white py-2 rounded font-bold mb-6 transition border border-gray-600">+ Inserir na Cerimônia</button>
